...
# porosity from GLHYMPS: Porosity = value, GLHYMPS_Shape_Area = area of each porosity value within watershed
porosity <- 
  read_csv(file.path("..", "Data", "GLHYMPS_Porosity_by_gauges.csv")) %>% 
  subset(GAGE_ID %in% sf_gages$STAID)

# calculate area-weighted mean transmissivity and total area
porosity_gage <-
  porosity %>% 
  group_by(GAGE_ID) %>% 
  summarize(area_m2 = sum(GLHYMPS_Shape_Area),
            Porosity_weightedAvg = weighted.mean(Porosity, GLHYMPS_Shape_Area)) %>% 
  mutate(S = Porosity_weightedAvg)  # could apply transformation if desired (i.e., S = 50% of porosity)

# there is a separate transmissivity file for each gage

# plot and inspect
p_S <-
  ggplot(porosity_gage, aes(x = S)) +
...
for (usgs_id in sf_gages$STAID){
  ...
  gage_S  <- porosity_gage$S[porosity_gage$GAGE_ID == usgs_id]
